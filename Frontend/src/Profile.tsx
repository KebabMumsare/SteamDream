...
function Profile() {
    ...
    return (
        <>
            ...
            <div
              className={`fixed inset-0 z-[60] bg-black/40 backdrop-blur-sm transition-opacity duration-200 max-[500px]:block hidden ${settingsOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
            ...
            <aside
              className={`fixed top-0 left-0 z-[70] h-screen w-[80vw] max-w-[320px] bg-[#1B2838] shadow-2xl transform transition-transform duration-300 ease-out rounded-r-2xl max-[500px]:block hidden ${settingsOpen ? 'translate-x-0' : '-translate-x-full'}`}
              role="dialog"
              aria-modal="true"
              aria-label="Settings"
            >
                    ...
                    <div className="sticky top-0 z-[48] bg-[#004E7B]">
                        ...
                        <div className="hidden max-[500px]:flex justify-center pb-3">
                            <button
                                ...
                                className="bg-[#66C0F4] hover:bg-[#2979A8] text-white font-mono rounded-full px-3 py-1.5 shadow transition-colors inline-flex items-center gap-2"
